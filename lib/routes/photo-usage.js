// Dado el token de sesión, dice cuántos créditos de IA ha gastado este mes de
// los incluidos en PRO, y cuántos créditos extra (comprados) le quedan.
// Solo informa, no descuenta nada.

const { emailFromRequest, isPro } = require('../auth');
const { redisCmd } = require('../redis');

const { MONTHLY_CREDITS } = require('../quota');
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ pro: true, used, limit: MONTHLY_CREDITS, extra });
  } catch (e) {
    res.status(500).json({ error: 'No se pudo comprobar tus créditos' });
  }
};
